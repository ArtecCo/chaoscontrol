  ...
  return (
    ...
    >
      ...
      <div className="card-list">
        {cards.map((card) => (
          <KanbanCardView key={card.id} card={card} onOpen={onOpenCard} />
        ))}
        {cards.length === 0 && <div className="empty-column">Drop a task here</div>}
      </div>

      <button className="add-card-button" type="button" onClick={() => onAddCard(column.id)}>
