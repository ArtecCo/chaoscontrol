...
function App() {
    ...
    return new Set(
      Object.values(board.cards)
        .filter((card) => {
          ...
          const priorityMatch = priority === "all" || card.priority === priority;
  ...
  const updateBoard = (updater: (current: Board) => Board) => {
    setData((current) => ({
      ...current,
      boards: current.boards.map((item) => item.id === board.id ? updater(item) : item),
    }));
  };
  ...
  const addColumn = () => {
    ...
    updateBoard((current) => ({
      ...
      columns: [
        ...current.columns,
        { id: createId("column"), title: title.trim(), color: "#64748b", cardIds: [] },
      ],
  ...
  const saveCard = (card: KanbanCard) => {
    updateBoard((current) => ({
      ...current,
      cards: { ...current.cards, [card.id]: card },
      updatedAt: new Date().toISOString(),
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    if (sourceColumn.id === destinationColumn.id) {
      ...
      const newIndex = destinationColumn.cardIds.indexOf(overId);
    ...
    updateBoard((current) => {
      ...
      return {
        ...current,
        columns: current.columns.map((column) => {
          if (column.id === source.id) return { ...column, cardIds: sourceCards };
          if (column.id === destination.id) return { ...column, cardIds: nextDestination };
          return column;
        }),
        updatedAt: new Date().toISOString(),
      };
  ...
  return (
    <div className="app-shell">
      <Sidebar
        ...
        onImport={() => document.getElementById("backup-input")?.click()}
        onSettings={() => window.alert("Settings will be expanded in the next phase.")}
      />

      {mobileSidebar && (
        <div className="mobile-sidebar-overlay" onClick={() => setMobileSidebar(false)}>
          <div onClick={(e) => e.stopPropagation()} className="mobile-sidebar">
            <div className="mobile-sidebar-header">
              <strong>Your boards</strong>
              <button className="icon-button" type="button" onClick={() => setMobileSidebar(false)}>
                <X size={18} />
              </button>
            </div>
            {data.boards.map((item) => (
              <button
                className={`mobile-board ${item.id === board.id ? "selected" : ""}`}
                key={item.id}
                type="button"
            ...
            <button className="mobile-board" type="button" onClick={addBoard}>
      ...
      <main className="main-content">
        ...
        <div className="page-content">
          <div className="page-heading">
            <div>
              <div className="breadcrumb">Workspace / Boards / {board.title}</div>
              <h1>{board.title}</h1>
              <p>{board.description || "Organize your work and personal tasks."}</p>
            </div>
            <div className="progress-summary">
              <span>{completedCards}/{totalCards} done</span>
          ...
          {search || priority !== "all" ? (
            <div className="filter-result">
              Showing {filteredCards.size} of {totalCards} tasks
              <button type="button" onClick={() => { setSearch(""); setPriority("all"); }}>Clear filters</button>
            </div>
          ) : null}
            ...
            <div className="kanban-board">
              {board.columns.map((column) => {
                const cards = column.cardIds
                  ...
                  .filter(Boolean)
                  .filter((card) => filteredCards.has(card.id));
                return (
                  <SortableContext
                    key={column.id}
              ...
              <button className="add-column-tile" type="button" onClick={addColumn}>
            ...
            <DragOverlay>
              {draggedCard ? (
                <div className="drag-preview">
                  ...
                  <span>{draggedCard.priority} priority</span>
                </div>
              ) : null}
            </DragOverlay>
          </DndContext>
