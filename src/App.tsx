...
import { CalendarDays, CheckCircle2, MoreHorizontal, Plus, X } from "lucide-react";
...
function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToUserData(auth.currentUser.uid, loadData(), (remote) => { lastSavedRef.current = JSON.stringify(remote); setData(remote); hydratedRef.current = true; setCloudReady(true); }, (error) => { hydratedRef.current = false; setCloudError(error.message); setCloudReady(false); });
    return unsubscribe;
  }, []);
  useEffect(() => {
    ...
    lastSavedRef.current = serialized; void saveUserData(auth.currentUser.uid, data).catch((error) => setCloudError(error instanceof Error ? error.message : "Cloud save failed."));
  }, [data]);
  ...
  const filteredCards = useMemo(() => {
    ...
    return new Set(Object.values(board.cards).filter((card) => (!query || card.title.toLowerCase().includes(query) || card.description.toLowerCase().includes(query) || card.tags.some((tag) => tag.toLowerCase().includes(query))) && (priority === "all" || card.priority === priority)).map((card) => card.id));
  ...
  const addBoard = () => { const title = window.prompt("Board name", "New board"); if (!title?.trim()) return; const stamp = new Date().toISOString(); const newBoard: Board = { id: createId("board"), title: title.trim(), description: "", columns: [{ id: createId("column"), title: "Backlog", color: "#94a3b8", cardIds: [] }, { id: createId("column"), title: "In Progress", color: "#eab308", cardIds: [] }, { id: createId("column"), title: "Done", color: "#22c55e", cardIds: [] }], cards: {}, createdAt: stamp, updatedAt: stamp }; setData((current) => ({ ...current, boards: [...current.boards, newBoard], activeBoardId: newBoard.id })); };
  ...
  const deleteColumn = (columnId: string) => { if (board.columns.length <= 1) return window.alert("A board needs at least one column."); const column = board.columns.find((item) => item.id === columnId); if (!column) return; if (column.cardIds.length > 0 && !window.confirm("This column contains issues. Delete it anyway?")) return; updateBoard((current) => { const cards = { ...current.cards }; column.cardIds.forEach((id) => delete cards[id]); return { ...current, cards, columns: current.columns.filter((item) => item.id !== columnId), updatedAt: new Date().toISOString() }; }); };
  ...
  const onDragEnd = (event: DragEndEvent) => { setDraggedCard(null); const activeId = String(event.active.id); const overId = event.over ? String(event.over.id) : null; if (!overId) return; const sourceColumn = board.columns.find((column) => column.cardIds.includes(activeId)); if (!sourceColumn) return; const destinationColumn = board.columns.find((column) => column.id === overId) ?? board.columns.find((column) => column.cardIds.includes(overId)); if (!destinationColumn) return; if (sourceColumn.id === destinationColumn.id) { const oldIndex = sourceColumn.cardIds.indexOf(activeId); const newIndex = destinationColumn.cardIds.indexOf(overId); if (newIndex < 0 || oldIndex === newIndex) return; updateBoard((current) => ({ ...current, columns: current.columns.map((column) => column.id === sourceColumn.id ? { ...column, cardIds: arrayMove(column.cardIds, oldIndex, newIndex) } : column), updatedAt: new Date().toISOString() })); return; } updateBoard((current) => { const source = current.columns.find((column) => column.id === sourceColumn.id)!; const destination = current.columns.find((column) => column.id === destinationColumn.id)!; const sourceCards = source.cardIds.filter((id) => id !== activeId); const targetIndex = destination.cardIds.indexOf(overId); const nextDestination = [...destination.cardIds.filter((id) => id !== activeId)]; nextDestination.splice(targetIndex >= 0 ? targetIndex : nextDestination.length, 0, activeId); return { ...current, columns: current.columns.map((column) => column.id === source.id ? { ...column, cardIds: sourceCards } : column.id === destination.id ? { ...column, cardIds: nextDestination } : column), updatedAt: new Date().toISOString() }; }); };

  if (!board) return <div className="workspace-empty"><div className="workspace-empty-mark">C</div><h2>Preparing your workspace</h2><p>No board exists yet.</p></div>;
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="page-content">
        <section className="board-header"><div className="board-heading"><div className="board-kicker">Project / {board.title}</div><div className="board-title-row"><h1>{board.title}</h1><span className="board-private">PRIVATE</span></div><p>{board.description || "Plan, track and complete work in one place."}</p></div><div className="board-header-actions"><button className="header-action" type="button" aria-label="Board actions"><MoreHorizontal size={20}/></button><button className="primary-button" type="button" onClick={()=>openAddTask()}><Plus size={17}/> Create issue</button></div></section>
        <nav className="workspace-tabs" aria-label="Workspace views"><button className={view==="board"?"active":""} type="button" onClick={()=>setView("board")}>Board</button><button className={view==="list"?"active":""} type="button" onClick={()=>setView("list")}>Issues</button><button type="button">Backlog</button><button type="button">Activity</button><span className="tab-spacer"/><span className="board-stat"><strong>{totalCards}</strong> issues</span><span className="board-stat"><strong>{completedCards}</strong> done</span><span className="board-stat"><strong>{progress}%</strong> complete</span></nav>
        ...
        {!cloudReady && firebaseConfigured ? <div className="filter-result">Connecting to your cloud workspace…</div> : null}{cloudError ? <div className="filter-result">Cloud sync error: {cloudError}</div> : null}
